...
Local uint32 GET_UINT32_OF_4UINT8(uint8* ptr);
Local void SET_4UINT8_TO_UINT32(uint8* ptr, uint32 value);
...
void* os_malloc(uint32 size)
{
   ...
   */
   ...
   {
      size      = GET_UINT32_OF_4UINT8(heap_ptr + HEAP_OFFSET_FOR_SIZE);
      /*used_size = GET_UINT32_OF_4UINT8(heap_ptr + HEAP_OFFSET_FOR_USED_SIZE);*/
...
void os_free(void* ptr)
{
      ...
      )
   {
      ...
      {
         SET_4UINT8_TO_UINT32((uint8*)heap_ptr + (uint32)HEAP_OFFSET_FOR_USED_SIZE, 0);/* set the used_size info the 0*/
      }
   }
}
Local uint32 GET_UINT32_OF_4UINT8(uint8* ptr)
